...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // SWECOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "SWECOIN Signed Message:\n";
...
static const int64 nTargetTimespan = 120; // SWECOIN: every 2 minutes
static const int64 nTargetSpacing = 60; // SWECOIN: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // SWECOIN: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal SWECOIN miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "SWECOIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // SWECOINd: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
